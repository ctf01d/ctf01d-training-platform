...
- Осталось:
  ...
  - История изменений состава/ролей (аудит‑лог действий).
...
- Осталось:
  - Планирование и статусы игр: Upcoming / Ongoing / Past.
...
- Осталось:
  ...
  - Историческая информация и метаданные (события/действия/состояния).

## Предлагаемые приоритеты (быстрые и заметные)

1. Ограничение капитанства: запрет "капитан только в одной команде".
2. Статусы игр и разделенный список: Upcoming / Ongoing / Past.
3. Снимок итогов: флаг завершения + immutable‑копия результатов и вывод на Scoreboard.
4. Writeups: модель и UI в игре/команде.
5. Сервисы: базовая валидация URL, поле `exploits_url`, бейджи статуса проверки (заглушка), скрыть Results гостям.
