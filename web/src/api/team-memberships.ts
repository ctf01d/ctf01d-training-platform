...
export type TeamMembershipCreate =
  components["schemas"]["TeamMembershipCreate"];
export type TeamMembershipUpdate =
  components["schemas"]["TeamMembershipUpdate"];
export type SetRoleRequest = components["schemas"]["SetRoleRequest"];

export async function listTeamMemberships(query?: {
  page?: number;
  per_page?: number;
}) {
...
export async function updateTeamMembership(
  ...
  return client.PATCH("/team-memberships/{id}", {
    params: { path: { id } },
    body,
  });
}
...
export async function approveTeamMembership(id: number) {
  return client.POST("/team-memberships/{id}/approve", {
    params: { path: { id } },
  });
}

export async function rejectTeamMembership(id: number) {
  return client.POST("/team-memberships/{id}/reject", {
    params: { path: { id } },
  });
}

export async function acceptTeamMembership(id: number) {
  return client.POST("/team-memberships/{id}/accept", {
    params: { path: { id } },
  });
}

export async function declineTeamMembership(id: number) {
  return client.POST("/team-memberships/{id}/decline", {
    params: { path: { id } },
  });
}
