...
export default function ResultsPage() {
  ...
  const [createForm, setCreateForm] = useState<ResultCreate>({ game_id: 0, team_id: 0, score: 0 })
  ...
  return (
    <div className="page">
      ...
      <div className="inline-form" style={{ marginBottom: 16 }}>
        ...
        />
        <button className="btn btn-sm" onClick={() => void fetchResults()}>Filter</button>
        {(filterGameId || filterTeamId) && (
          <button className="btn btn-sm" onClick={() => { setFilterGameId(''); setFilterTeamId('') }}>Clear</button>
        )}
      </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} className="create-form">
          <div className="form-group">
            <label>Game ID</label>
            <input type="number" value={createForm.game_id || ''} onChange={e => setCreateForm(f => ({ ...f, game_id: Number(e.target.value) }))} required />
          </div>
          <div className="form-group">
            <label>Team ID</label>
            <input type="number" value={createForm.team_id || ''} onChange={e => setCreateForm(f => ({ ...f, team_id: Number(e.target.value) }))} required />
          </div>
          <div className="form-group">
            <label>Score</label>
            <input type="number" value={createForm.score ?? ''} onChange={e => setCreateForm(f => ({ ...f, score: Number(e.target.value) }))} />
          </div>
      ...
      {editingId !== null && (
        <form onSubmit={e => { e.preventDefault(); void handleSave() }} className="edit-form">
          <div className="form-group">
            <label>Score</label>
            <input type="number" value={editForm.score ?? ''} onChange={e => setEditForm(f => ({ ...f, score: Number(e.target.value) }))} />
          </div>
