...
export default function UniversitiesPage() {
  ...
  const fetchUniversities = useCallback(async () => {
    setLoading(true)
    setError(null)
    const { data, error: err } = await universitiesApi.listUniversities({ page, per_page: perPage })
    if (err) {
      setError(err)
    } else if (data) {
  ...
  const startEdit = (u: University) => {
    setEditingId(u.id)
    setEditForm({ name: u.name ?? undefined, site_url: u.site_url ?? undefined, avatar_url: u.avatar_url ?? undefined })
  }

  const handleSave = async () => {
    if (editingId === null) return
    setSaving(true)
    const { error: err } = await universitiesApi.updateUniversity(editingId, editForm)
    setSaving(false)
    if (err) { setError(err); return }
    setEditingId(null)
    await fetchUniversities()
    ...
    <div className="page">
      ...
      {showCreate && (
        <form onSubmit={handleCreate} className="create-form">
          ...
          <div className="form-group">
            ...
            <input value={createForm.site_url ?? ''} onChange={e => setCreateForm(f => ({ ...f, site_url: e.target.value }))} />
          </div>
          <div className="form-group">
            <label>Avatar URL</label>
            <input value={createForm.avatar_url ?? ''} onChange={e => setCreateForm(f => ({ ...f, avatar_url: e.target.value }))} />
          </div>
      ...
      {editingId !== null && (
        <form onSubmit={e => { e.preventDefault(); void handleSave() }} className="edit-form">
          ...
          <div className="form-group">
            <label>Site URL</label>
            <input value={editForm.site_url ?? ''} onChange={e => setEditForm(f => ({ ...f, site_url: e.target.value }))} />
          </div>
          <div className="form-group">
            <label>Avatar URL</label>
